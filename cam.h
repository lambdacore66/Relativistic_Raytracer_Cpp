...
class cam {
	...
	public:
		...
		ray get_ray(int i, int j, double emission_time);
...
ray cam::get_ray(int i, int j, double emission_time) {
	...
	vec4 ray_orig(position.x(), position.y(), position.z(), emission_time);
	return ray(ray_orig, ray_dir.normalize());
